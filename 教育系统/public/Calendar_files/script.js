			...
			for ( var i = 0; i < 7; i++ ) {
				...
				for ( var j = 0; j <= 6; j++ ) {
					...
					if ( day <= monthLength && ( i > 0 || j >= p ) ) {
						...
						inner += '<span class="fc-date">' + day + '</span><span class="off"></span><span class="fc-weekday">' + this.options.weekabbrs[ j + this.options.startIn > 6 ? j + this.options.startIn - 6 - 1 : j + this.options.startIn ] + '</span><div class="shezhi"><ul><li>设为上班</li><li>设为放假</li></ul></div>';
			...
			$(function() {
				...
				var cal = $( '#calendar' ).calendario( {
						onDayClick : function( $el, $contentEl, dateProperties ) {
							...
							if(flag==true){
							$el.children(".shezhi").css("visibility","visible")
								flag=false
                            }else{
								flag=true;
								$el.children(".shezhi").css("visibility","hidden")
							}
							//}
                            $(document).on("click",".shezhi ul li",function () {
								if($(this).text()=="设为上班"){
                                    $(this).parent().parent().siblings(".off").text("上班")
									console.log($(this).parent().parent().siblings(".off"))
                                }else{
                                    $(this).parent().parent().siblings(".off").text("放假")
                                }
                            });
